...
export function FollowButton({ userId }) {
  ...
  useEffect(() => {
    ...
    const fetchFollowStatus = async () => {
      try {
        const response = await fetch("http://localhost:8000/api/follows/" + encodeURIComponent(userId) + "/status/", {
          headers: {
  ...
  const handleFollowToggle = async () => {
    ...
    setError(null);
    try {
      const url = "http://localhost:8000/api/follows/" + encodeURIComponent(userId) + "/" + (isFollowing ? "unfollow" : "follow") + "/";
